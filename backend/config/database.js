...
if (process.env.DATABASE_URL || process.env.NEON_DATABASE_URL) {
  ...
  if (!process.env.DB_PASSWORD) {
    ...
    console.warn('Example: NEON_DATABASE_URL=postgresql://user:pass@host/db?sslmode=require\n');
  }
...
const testConnection = async () => {
  try {
    ...
    const dbType = (process.env.DATABASE_URL || process.env.NEON_DATABASE_URL) ? 'Neon/Cloud' : 'Local';
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.warn('⚠️  Continuing despite database connection error (development mode)');
    }
    return false;
  }
};
...
pool.on('error', (err) => {
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.warn('⚠️  Continuing despite database pool error (development mode)');
  }
});
