...
const testRemoteBackend = async () => {
  if (currentBaseURL === PRODUCTION_API_URL) {
    try {
      ...
      console.warn(
        "⚠️ Remote backend not available, switching to local backend"
      );
...
export const adminAPI = {
  ...
  getAllTests: (page = 1, limit = 10, category) =>
    api.get("/admin/tests", { params: { category, page, limit } }), // Admin: Get all tests including private
